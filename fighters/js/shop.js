...
function loadShopItems() {
    ...
    SHOP_ITEMS.forEach(item => {
        ...
        const active = item.effect === 'background' && gameState.customBackground === item.bgId;
        const div = document.createElement('div');
        div.className = 'shop-item' + (active ? ' active-background' : '');
        div.innerHTML = `
            <div class="item-name">${item.name}</div>
            <div class="item-desc">${item.description}</div>
            <div class="item-price">${item.price} COINS</div>
            <button class="buy-btn" data-id="${item.id}" ${owned || !canAfford ? 'disabled' : ''}>${owned ? (item.effect === 'background' ? (active ? 'ACTIVE' : 'SELECT') : 'OWNED') : (canAfford ? 'BUY NOW' : 'NEED COINS')}</button>
            ${owned ? `<div class="owned-badge">${item.effect === 'background' ? (active ? 'ACTIVE' : 'OWNED') : 'OWNED'}</div>` : ''}
        `;
...
function buyItem(id) {
    ...
    if (item.type === 'permanent') {
        ...
        if (id === 'bossUnlock') { gameState.bossUnlocked = true; localStorage.setItem('boss67Unlocked', 'true'); alert('67 BOSS UNLOCKED!'); }
        else if (item.effect === 'background') { gameState.customBackground = item.bgId; localStorage.setItem('customBackground', item.bgId); applyCustomBackground(); }
    } else {
...
function applyShopEffects() {
    ...
    if (inv.damageBoost) gameState.player.damageMultiplier = 1.2;
    if (inv.healthBoost && gameState.player.character) { gameState.player.maxHealth = Math.floor(gameState.player.character.hp * 1.15); gameState.player.health = gameState.player.maxHealth; }
    if (inv.comboMaster) gameState.player.comboMultiplier = 1.25;
    if (inv.parryCharm && gameState.cpu && gameState.cpu.difficulty) gameState.cpu.difficulty.parryChance *= 0.8;
    if (inv.doubleCoins && inv.doubleCoins > 0) gameState.coinMultiplier = 2;
}
function updateDoubleCoins() {
    if (gameState.playerInventory.doubleCoins > 0) {
        gameState.playerInventory.doubleCoins--;
        if (gameState.playerInventory.doubleCoins <= 0) { delete gameState.playerInventory.doubleCoins; alert('Double Coins effect has expired!'); }
        localStorage.setItem('playerInventory', JSON.stringify(gameState.playerInventory));
    }
}
